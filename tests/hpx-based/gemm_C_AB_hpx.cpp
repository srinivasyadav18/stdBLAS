...
template<class ExPolicy, class A_t, class B_t, class C_t>
void hpx_blas_gemm_impl(ExPolicy policy, A_t A, B_t B, C_t C)
{
  ...
  auto B_preKernel = hpxtesting::create_stdvector_and_copy_rowwise(B);
  auto C_preKernel = hpxtesting::create_stdvector_and_copy_rowwise(C);

  // compute gold gemm
  std::vector<value_type> gold(extent0*extent2);
  using mdspan_t = mdspan<value_type, extents<std::size_t, dynamic_extent, dynamic_extent>>;
  mdspan_t C_gold(gold.data(), extent0, extent2);
  gemm_gold_solution(A, B, C_gold);

  stdla::matrix_product(policy, A, B, C);
