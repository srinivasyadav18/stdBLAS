...
struct gemm_perf_test
{
    ...
    using aligned_allocator_t = boost::alignment::aligned_allocator<float, 4>;
    using vector_t = std::vector<T, aligned_allocator_t>;
    ...
    {  
        vector_t x_data(n*n), y_data(n*n), z_data(n*n);
...
struct gemm_simd
{
    ...
    void operator()(auto policy, auto const& a, auto const&b, auto& c)
    {
        ...
        constexpr size_t UnrollOuterloop = 4;
        size_t N = a.extent(0);
        size_t i0 = N / UnrollOuterloop * UnrollOuterloop;

        for (size_t i = 0; i < i0; i += UnrollOuterloop) {
            for (size_t j = 0; j < N; j += V::size()) {
                V c_ij[UnrollOuterloop];
                for (size_t n = 0; n < UnrollOuterloop; ++n) {
                    c_ij[n] = a(i+n,0) * V(&b(0,j),std::experimental::vector_aligned);
                }
                for (size_t k = 1; k < N - 1; ++k) {
                    for (size_t n = 0; n < UnrollOuterloop; ++n) {
                        c_ij[n] += a(i+n,k) * V(&b(k,j),std::experimental::vector_aligned);
                    }
                }
                for (size_t n = 0; n < UnrollOuterloop; ++n) {
                    c_ij[n] += a(i+n,N-1) * V(&b(N-1,j),std::experimental::vector_aligned);
                    c_ij[n].copy_to(&c(i+n,j),std::experimental::vector_aligned);
                }
            }
        }
        ...
        for (size_t j = 0; j < N; j += V::size()) {
            ...
            for (size_t n = i0; n < N; ++n) {
                c_ij[n - i0] = a(n,0) * V(&b(0,j),std::experimental::vector_aligned);
            }
            for (size_t k = 1; k < N - 1; ++k) {
                for (size_t n = i0; n < N; ++n) {
                    c_ij[n - i0] += a(n,k) * V(&b(k,j),std::experimental::vector_aligned);
                }
            }
            for (size_t n = i0; n < N; ++n) {
                c_ij[n - i0] += a(n,N-1) * V(&b(N-1,j),std::experimental::vector_aligned);
                c_ij[n - i0].copy_to(&c(n,j),std::experimental::vector_aligned);
            }
        }
    }
};

struct gemm_simd_unrolled_hpx
{
    using V = std::experimental::native_simd<float>;
    ...
    void operator()(auto policy, auto const& a, auto const&b, auto& c)
    {
        ...
        hpx::experimental::for_loop_strided(
            hpx::execution::experimental::to_non_simd(policy),
            std::size_t(0), i0, UnrollOuterloop, [&](auto i){
            // for (size_t i = 0; i < i0; i += UnrollOuterloop) {
                for (size_t j = 0; j < N; j += V::size()) {
                    V c_ij[UnrollOuterloop];
                    for (size_t n = 0; n < UnrollOuterloop; ++n) {
...
struct gemm_simd_hpx
{
    ...
    void operator()(auto policy, auto const& a, auto const&b, auto& c)
    {
        ...
        hpx::experimental::for_loop(
            hpx::execution::experimental::to_non_simd(policy),
            std::size_t(0), N, [&](auto i){
                for (size_t j = 0; j < N; j += V::size()) {
                    V c_ij = a(i,0) * V(&b(0,j),std::experimental::vector_aligned);
                    for (size_t k = 1; k < N - 1; ++k) {
                        c_ij += a(i,k) * V(&b(k,j),std::experimental::vector_aligned);
                    }
                    c_ij += a(i,N-1) * V(&b(N-1,j),std::experimental::vector_aligned);
                    c_ij.copy_to(&c(i,j),std::experimental::vector_aligned);
                }
        });
    }
};
...
template <int l1=32, int l2=256>
struct gemm_L2
{
    void operator()(auto policy, auto const& A, auto const&B, auto& C)
    {
        // const int BLOCK_SIZE=std::min(int(A.extent(0)), l1);

        for (std::size_t l2_i = 0; l2_i < C.extent(0); l2_i += l2) {
            for (std::size_t l2_j = 0; l2_j < C.extent(1); l2_j += l2) {

                for (std::size_t l1_i = l2_i; l1_i < l2_i + l2; l1_i += l1) {
                    for (std::size_t l1_j = l2_j; l1_j < l2_j + l2; l1_j += l1) {

                        for (std::size_t i = l1_i; i < l1_i + l1; i++) 
                            for (std::size_t j = l1_j; j < l1_j + l1; j++) 
                                C(i, j) = {};
                        for (std::size_t l1_k = 0; l1_k < A.extent(1); l1_k += l1) {
                            for (std::size_t i = l1_i; i < l1_i + l1; i++) {
                                for (std::size_t j = l1_j; j < l1_j + l1; j++) {
                                    for (std::size_t k = l1_k; k < l1_k + l1; k++) {
                                        C(i,j) += A(i,k) * B(k,j);
                                    }
                                }
                            }
...
struct gemm_L2_hpx
{
    void operator()(auto policy, auto const& A, auto const&B, auto& C)
    {
        ...
        hpx::experimental::for_loop_strided(
            hpx::execution::experimental::to_non_simd(policy),
            std::size_t(0), C.extent(0), l2, [&](auto l2_i){
                // for (std::size_t l2_i = 0; l2_i < C.extent(0); l2_i += l2) {
                for (std::size_t l2_j = 0; l2_j < C.extent(1); l2_j += l2) {
...
int hpx_main()
{
    std::cout << std::experimental::native_simd<float>::size() << '\n';

    int start = 3, end = 12, iters=2;  

    std::cout << gemm_simd::V::size() << '\n';

    gemm_perf_test<float, gemm_L2_hpx<32, 256>>("gemm_L2_hpx<32, 256>", gemm_L2_hpx<32, 256>{}, 8, 13, iters);

    gemm_perf_test<float, gemm_L2<32, 256>>("gemm_L2<32, 256>", gemm_L2<32, 256>{}, 8, end, iters);
    return hpx::finalize();

    gemm_perf_test<float, gemm_simd_hpx>("gemm_simd_hpx", gemm_simd_hpx{}, 8, end, iters);

    gemm_perf_test<float, gemm_simd_unrolled_hpx>("gemm_simd_unrolled_hpx", gemm_simd_unrolled_hpx{}, 8, end, iters);
    gemm_perf_test<float, gemm_simd>("gemm_simd", gemm_simd{}, 8, end, iters);
